...
async function handleCreateRace() {
	// render starting UI
	renderAt('#race', renderRaceStartView({name: 'test'}))
	...
	race.then((race) => {
		...
		if (raceId === 2 ) {
			...
			.then((race) => {
				raceId = race.ID - 1 // FIX API
				store.race_id = raceId
			})
		}
		else {
			store.race_id = raceId - 1 // FIX API
		}
	})
	.then(() => {
...
function getRacers() {
	...
	try {
		...
		console.log(`Got error getting cars:`, error)
	}
}
...
function getRace(id) {
	...
	try {
		...
		console.log(`Got error getting race:`, error)
	}
}
